...
class CreateFrontendSettingsTable extends Migration
{
    ...
    Schema::create('frontend_settings', function (Blueprint $table) {
        ...
        if (FrontendSetting::count() === 0) {
            $categoryIds = Service::pluck('id')->take(6)->map(function ($id) {
                return (string) $id;
            })->toArray();
            ...
            $officeIds = Office::where('status', true)->pluck('id')->take(6)->map(function ($id) {
                return (string) $id;
            })->toArray();
            $settingsData = [
                [
                    ...
                    'key' => 'section_1',
                    'status' => '1',
                    'value' => '{"section_1":1,"title":"Your Instant Connection To Right Handyman Service","description":"Get things done with our reliable handyman service. From repairs to installations, weve got you covered. Your go-to solution for household tasks.","current_location":"on","enable_search":"on","enable_post_job":"on","enable_popular_services":"on","category_id":'.json_encode($serviceIds).',"enable_popular_office":"on","provider_id":'.json_encode($officeIds).'}',
                ],
                [
                    'type' => 'landing-page-setting',
                    'key' => 'section_2',
                    'status' => '1',
                    'value' => '{"section_2":1,"title":"Our Popular Categories","category_id":'.json_encode($serviceIds).'}',
                ],
                [
                    'type' => 'landing-page-setting',
                    'key' => 'section_3',
