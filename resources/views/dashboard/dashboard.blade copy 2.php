...
<style>
    ...
    .map-container {
        ...
        border-radius: 12px;
        overflow: hidden;
        background: linear-gradient(135deg, #ffcc00, #ff9966); /* تدرج اللون الفاتح */
    ...
    .map-footer {
        ...
        font-size: 17px;
        color: #fff;
        background: #ff9966; /* تدرج من اللون البرتقالي الفاتح */
        border-radius: 0 3 10px 10px;
    }

    /* تنسيق معلومات السائق */
    .driver-info {
        font-size: 14px;
        font-weight: bold;
        color: #333;
...
<script>
    ...
    function initMap() {
        ...
        fetchDriverLocations(); // جلب المواقع عند التحميل
        setInterval(fetchDriverLocations, 300000); // تحديث تلقائي كل 10 ثوانٍ
    }
    ...
    function fetchDriverLocations() {
        fetch("{{ route('live-drivers-locations') }}")
                ...
                data.forEach(driver => {
                    ...
                    if (markers[driverId]) {
                        ...
                        let marker = new google.maps.Marker({
                            ...
                            title: `🚖 سائق رقم: ${driverId}`,
                        });

                        // عرض المعلومات عند تمرير الماوس
            ...
            .catch(error => console.error("❌ خطأ أثناء جلب المواقع:", error));
    }
    ...
    function updateLastUpdatedTime() {
        ...
        document.getElementById("last-update").innerText = "آخر تحديث: " + formattedTime;
    }
    ...
    // async function getRegionName(latitude, longitude) {
    //             const API_KEY = "YOUR_GOOGLE_MAPS_API_KEY"; // 🔹 ضع مفتاح API الخاص بك هنا
    ...
    //                             return component.long_name; // 🔹 إعادة اسم المحافظة فقط
    //                         }
    //                     }
    //                 }
    //                 return "غير معروف"; // 🔹 في حال لم يتم العثور على محافظة
    //             } catch (error) {
    ...
    //         getRegionName(33.5138, 36.2765).then(region => console.log(region));
