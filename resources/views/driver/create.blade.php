    ...
    <div class="container-fluid">
        <div class="row">
                ...
                <div class="card card-block card-stretch">
                    <div class="card-body p-0">
                        <div class="d-flex justify-content-between align-items-center p-3 flex-wrap gap-3">
                            <h5 class="font-weight-bold">{{ isset($driver) ? __('messages.Edit_Driver') : __('messages.Add_New_Driver') }}</h5>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <form  id="driverForm" method="POST" action="{{ isset($driver) ? route('driver.store', $driver->id) : route('driver.store') }}" enctype="multipart/form-data">
                            @csrf
                            @if (isset($driver->id))
                                ...
                                <div class="card-body p-1">
                                    <div class="d-flex flex-column align-items-center my-4">
                                        <div class="border p-2" style="height: 150px; width: 150px; border-radius: 50%; overflow: hidden; display: flex; justify-content: center; align-items: center;">
                                            <img id="imagePreview" src="{{ $driver->photo ?? get_default_image('driver') }}" alt="Preview" style="height: 100%; width: 100%; object-fit: cover; border-radius: 50%;">
                                        </div>
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <input type="text" name="firstName" id="firstName" class="form-control" value="{{ old('firstName', $driver->firstName ?? '') }}" required>
                                    @error('firstName')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <input type="text" name="lastName" id="lastName" class="form-control" value="{{ old('lastName', $driver->lastName ?? '') }}" required>
                                    @error('lastName')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <input type="text" name="phoneNumber" id="phoneNumber" class="form-control" value="{{ old('phoneNumber', $driver->phoneNumber ?? '') }}" required pattern="[0-9]{10}">
                                    @error('phoneNumber')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                ...
                                @if(!isset($driver->id))
                                    <div class="form-group col-md-4">
                                        <label for="password" class="form-control-label">{{ __('messages.password') }} <span class="text-danger">*</span></label>
                                    ...
                                    <div class="form-group col-md-4">
                                        <label for="password_confirmation" class="form-control-label">{{ __('messages.confirm_password') }} <span class="text-danger">*</span></label>
                                        <div class="input-group">
                                ...
                                <div class="form-group col-md-4">
                                    <label for="vehicleId" class="form-control-label">{{ __('messages.vehicle_number') }} <span class="text-danger">*</span></label>
                                    <select name="vehicleId" id="vehicleId" class="select2js form-control" required>
                                        <option value="">{{ __('messages.select_name', ['select' => __('messages.vehicle_number')]) }}</option>
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <select name="city" id="city" class="select2js form-control" required>
                                        ...
                                        @foreach($cities as $city)
                                            <option value="{{ $city->name }}" {{ old('city', $driver->city ?? '') == $city->name ? 'selected' : '' }}>
                                                {{ $city->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <input type="text" name="region" id="region" class="form-control" value="{{ old('region', $driver->region ?? '') }}" required>
                                    @error('region')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                            ...
                            <button type="submit" id="submitBtn" class="btn btn-md btn-primary float-right">
                                {{ $driver->firstName !=null ? __('messages.Update_Driver') : __('messages.Add_New_Driver') }}
                            </button>
                            <script>
