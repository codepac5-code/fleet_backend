        ...
        <div class="row">
                      ...
                      <div class="row">
                        <div class="form-group col-md-4">
                          ...
                          <input type="text" name="name" id="name" value="{{ old('name', $subservice->name ?? '') }}" 
                            placeholder="{{ __('messages.name_arabic') }}" class="form-control required-field">
                          @error('name')
                        ...
                        <div class="form-group col-md-4">
                          ...
                          <input type="text" name="name_en" id="name_en" value="{{ old('name_en', $subservice->name_en ?? '') }}" 
                            placeholder="{{ __('messages.name_en') }}" class="form-control" required>
                          @error('name_en')
                            <small class="text-danger">{{ $message }}</small>
                          @enderror
                        </div>
                        ...
                        <div class="col-md-12">
                          ...
                          <div id="travelRoutesContainer">
                            <div class="route-item mb-4">
                              <div class="row">
                                <div class="form-group col-md-4">
                                  <label for="departureCity_0" class="form-control-label">
                                    {{ __('messages.departure_city') }} <span class="text-danger">*</span>
                                  </label>
                                  <select name="routes[0][departureCity]" id="departureCity_0" class="select2js form-control" required>
                                    ...
                                    @foreach($cities as $city)
                                      <option value="{{ $city->name }}" 
                                        {{ old('routes.0.departureCity', $subservice->routes[0]['departureCity'] ?? '') == $city->name ? 'selected' : '' }}>
                                        {{ $city->name }}
                                      </option>
                                    @endforeach
                                  </select>
                                ...
                                <div class="form-group col-md-4">
                                  <label for="arrivalCity_0" class="form-control-label">
                                    {{ __('messages.arrival_city') }} <span class="text-danger">*</span>
                                  </label>
                                  <select name="routes[0][arrivalCity]" id="arrivalCity_0" class="select2js form-control" required>
                                    ...
                                    @foreach($cities as $city)
                                      <option value="{{ $city->name }}" 
                                        {{ old('routes.0.arrivalCity', $subservice->routes[0]['arrivalCity'] ?? '') == $city->name ? 'selected' : '' }}>
                                        {{ $city->name }}
                                      </option>
                                    @endforeach
                                  </select>
                                ...
                                <div class="form-group col-md-3">
                                  <label for="tripPrice_0" class="form-control-label">
                                    {{ __('messages.trip_price') }} <span class="text-danger">*</span>
                                  </label>
                                  <div class="input-group">
                                    <input type="number" id="tripPrice_0" name="routes[0][tripPrice]" class="form-control" 
                                      step="100" min="0" value="{{ old('routes.0.tripPrice', $subservice->routes[0]['tripPrice'] ?? 0.00) }}" required>
                          ...
                          <div class="form-group">
                            <button type="button" id="addRoute" class="btn btn-primary">
                              <i class="fas fa-plus"></i> {{ __('messages.add_new_route') }}
                            </button>
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="row" id="regularServiceMessage" style="display: none;">
                        <div class="col-md-12">
                          <div class="alert alert-success mb-3">
                            {{ __('messages.regular_service') }}
                          </div>
                        </div>
                        <div class="form-group col-md-3">
                          <label for="openPrice">{{ __('messages.open_price') }} <span class="text-danger">*</span></label>
                          <input type="number" id="openPrice" name="openPrice" class="form-control price-input" 
                            step="100" min="0" value="{{ old('openPrice', $subservice->openPrice ?? 0.00) }}">
                          @error('openPrice')
                            <small class="text-danger">{{ $message }}</small>
                          @enderror
                        </div>
                        ...
                        <div class="form-group col-md-3">
                          ...
                          <input type="number" id="kmPrice" name="kmPrice" class="form-control price-input" 
                            step="50" min="0" value="{{ old('kmPrice', $subservice->kmPrice ?? 0.00) }}">
                          @error('kmPrice')
                            <small class="text-danger">{{ $message }}</small>
                          @enderror
                        </div>
                        ...
                        <div class="form-group col-md-3">
                          ...
                          <input type="number" id="minutePrice" name="minutePrice" class="form-control price-input" 
                            step="5" min="0" value="{{ old('minutePrice', $subservice->minutePrice ?? 0.00) }}">
                          @error('minutePrice')
                            <small class="text-danger">{{ $message }}</small>
                          @enderror
                        </div>
                      ...
                      <div class="row mt-3">
                        <div class="form-group col-md-6">
                          ...
                          <textarea name="description" class="form-control textarea" rows="3" 
                            placeholder="{{ __('messages.description') }}">{{ old('description', $subservice->description) }}</textarea>
                          @error('description')
                            <small class="text-danger">{{ $message }}</small>
                          @enderror
                        </div>
                        ...
                        <div class="form-group col-md-6">
                          ...
                          <textarea name="description_en" class="form-control textarea" rows="3" 
                            placeholder="{{ __('messages.description_en') }}">{{ old('description_en', $subservice->description_en) }}</textarea>
                ...
                $(document).ready(function() {
                    ...
                    $('#addRoute').click(function() {
                        ...
                        <div class="route-item mb-4">
                            <div class="row">
                                <div class="form-group col-md-4">
                                    <label for="departureCity_${routeCounter}" class="form-control-label">
                                        {{ __('messages.departure_city') }} <span class="text-danger">*</span>
                                    </label>
                                ...
                                <div class="form-group col-md-4">
                                    <label for="arrivalCity_${routeCounter}" class="form-control-label">
                                        {{ __('messages.arrival_city') }} <span class="text-danger">*</span>
                                    </label>
                                ...
                                <div class="form-group col-md-3">
                                    <label for="tripPrice_${routeCounter}" class="form-control-label">
                                        {{ __('messages.trip_price') }} <span class="text-danger">*</span>
                                    </label>
                                    <div class="input-group">
                                        <input type="number" id="tripPrice_${routeCounter}" name="routes[${routeCounter}][tripPrice]" class="form-control" 
                                            step="100" min="0" value="0.00" required>
                                        <div class="input-group-append">
                                            <button class="btn btn-outline-danger remove-route" type="button">
                                                <i class="fas fa-trash"></i>
                                            </button>
                                        </div>
                                    </div>
            ...
            $(document).ready(function() {
                ...
                $serviceSelect.on('change', function() {
                    ...
                    const isTravelService = selectedOption.data('travel-service') == true || 
                                          selectedOption.data('travel-service') == 1;
            
                    // Hide all first
                    $priceInputsContainer.hide();
                    $regularServiceMessage.hide();
                    $priceInputs.prop('required', false);
            
                    // Show appropriate section
                    if (isTravelService) {
                        $priceInputsContainer.show();
                        $priceInputs.prop('required', true);
                    } else {
                        $regularServiceMessage.show();
                    }
                });
            
                // Trigger on page load if there's a preselected value
