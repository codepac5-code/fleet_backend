...
<div class="p-4">
  <div class="stats-box grid md:grid-cols-3 gap-4">

    <!-- إجمالي الرحلات -->
    <div class="stat flex flex-col gap-2">
      <div class="flex items-center justify-between">
        <span class="text-gray-500">إجمالي الرحلات</span>
        <i data-lucide="layers"></i>
      </div>
      <div id="totalTrips" class="text-3xl font-bold text-indigo-600">0</div>
    </div>

    <!-- الإيرادات -->
    <div class="stat flex flex-col gap-2">
      <div class="flex items-center justify-between">
        <span class="text-gray-500">الإيرادات</span>
        <i data-lucide="dollar-sign"></i>
      </div>
      <div id="revenue" class="text-3xl font-bold text-orange-500">{{ getPriceFormat(0) }}</div>
    </div>

    <!-- المسافة -->
    <div class="stat flex flex-col gap-2">
      <div class="flex items-center justify-between">
        <span class="text-gray-500">إجمالي المسافة</span>
        <i data-lucide="route"></i>
      </div>
      <div id="totalDistance" class="text-3xl font-bold text-green-600">0 km</div>
    </div>
...
async function fetchTrips(reset = false){
  try {
    ...
    if(driverSelect.getValue())
    params.append("driver", driverSelect.getValue());
    ...
    const res = await fetch(`/scheduled-ride-data?` + params.toString());
...
function renderActiveFilters(){
  ...
  const driverId = driverSelect.getValue?.();

  if(driverId){
    const item = driverSelect.options?.[driverId] || {};

    filters.push({
      key: 'driver',
      label: item?.name || "سائق",
      icon: 'user',
      value: driverId
    });
  }
  ...
  if(fromFilter.value){
    const text = fromFilter.options[fromFilter.selectedIndex]?.text || "من";
  ...
  if(toFilter.value){
    const text = toFilter.options[toFilter.selectedIndex]?.text || "إلى";
...
if(key === 'driver') driverSelect.clear();
...
// TomSelect reset
  driverSelect.clear(true);
  driverSelect.setValue("");
  driverSelect.clear(true);

  fromFilter.innerHTML = `<option value="">اختر مدينة الانطلاق</option>`;
  toFilter.innerHTML = `<option value="">اختر مدينة الوصول</option>`;
  ...
  <div class="card min-w-[340px] p-0 overflow-hidden border shadow-xl hover:shadow-2xl transition-all duration-300">
    ...
    <div class="border-t p-5 bg-gray-50 space-y-5">
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex-1">
          <div class="font-semibold">${t.driver?.name}</div>
...
async function confirmAssignDriver(){
  ...
  const res = await fetch(`/trips/assign-driver`, {
    method: 'POST',
    headers: {
...
async function loadDrivers(search=''){
  ...
  const res = await fetch(`/drivers?search=${search}`);
  const json = await res.json();
...
async function confirmCancelTrip(){
  ...
  const res = await fetch(`/trips/cancel`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "Accept": "application/json",
...
const driverSelect = new TomSelect("#driverFilter",{
  valueField: "id",
  labelField: "name",
  searchField: "name",
    ...
    option: function(data, escape){
      return `
        <div class="flex items-center gap-3 p-2">
          ...
          <img src="${data.image || 'https://i.pravatar.cc/80'}"
...
function renderStats(){
  ...
  document.getElementById("totalDistance").innerText = json.total_distance || "0 km";
}
...
[datePicker,fromFilter,toFilter].forEach(el=>{
