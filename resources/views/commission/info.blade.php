...
<div class="card-body p-0">
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6 align-text">
                    <span id="payment_status__span"
                        class="{{ optional($bookingdata->paymentStatus) == 'paid' ? 'text-success' : 'text-danger' }}">
                        {{-- {{ ucwords(str_replace('_', ' ',  optional($bookingdata->paymentStatus) ?: 'pending'))}} --}}
                    </span>
                </div>
            </div>
            <div class="row">
    ...
    <div class="py-3 d-flex gap-3 flex-wrap customer-info-detail mb-2">
        <div class="c1-light-bg radius-10 py-3 px-4 flex-grow-1">
            ...
            <h5 class="c1 mb-3">{{optional($bookingdata->user)->firstName ?? '-'}}</h5>
            <ul class="list-info">
                <li>
                    <span class="material-icons customer-info-text">{{__('messages.phone_information')}}</span>
                    <a href="tel:{{optional($bookingdata->user)->phoneNumber}}" class="customer-info-value">
                        <p class="mb-0">{{ optional($bookingdata->user)->phoneNumber ?? '-' }}</p>
                    </a>
                </li>
                <li>
                    <span class="material-icons  customer-info-text">{{__('messages.address')}}</span>
                    <p class="customer-info-text">{{ optional($bookingdata->user)->address ?? '-' }}</p>
                </li>
            </ul>
        </div>
        ...
        <div class="c1-light-bg radius-10 py-3 px-4 flex-grow-1">
            ...
            <h5 class="c1 mb-3">{{optional($bookingdata->office)->officeName ?? '-'}}</h5>
            <ul class="list-info">
                <li>
                    <span class="material-icons customer-info-text">{{__('messages.phone_information')}}</span>
                    <a href="tel:{{optional($bookingdata->user)->phoneNumber}}" class="customer-info-value">
                        <p class="mb-0">{{ optional($bookingdata->office)->contactNumber ?? '-' }}</p>
                    </a>
                </li>
                <li>
                    <span class="material-icons customer-info-text">{{__('messages.address')}}</span>
                    <p class="customer-info-text">{{ optional($bookingdata->office)->address ?? '-' }}</p>
                </li>
            </ul>
        </div>
        ...
        <div class="c1-light-bg radius-10 py-3 px-4 flex-grow-1">
            ...
            <h5 class="c1 mb-3">{{optional($bookingdata->driver)->firstName ?? '-'}}</h5>
            <ul class="list-info">
                <li>
                    <span class="material-icons  customer-info-text">{{__('messages.phone_information')}}</span>
                    <a href="" class=" customer-info-value">
                        <p class="mb-0">{{optional($bookingdata->driver)->phoneNumber ?? '-'}}</p>
                    </a>
                </li>
                <li>
                    <span class="material-icons  customer-info-text">{{__('messages.address')}}</span>
                    <p class=" customer-info-value">{{optional($bookingdata->driver)->address ?? '-'}}</p>
                </li>
            </ul>
            {{-- @endforeach --}}
        </div>
                ...
                <table class="table-sm title-color align-right w-100">
                    <tbody>
                        <tr>
                            ...
                            <td class="bk-value">{{$bookingdata->subService->openPrice }} * {{$bookingdata->subService->kmPrice}}  * {{$bookingdata->subService->minutePrice }}  = {{getPriceFormat($bookingdata->amount)}}</td>
                        </tr>
