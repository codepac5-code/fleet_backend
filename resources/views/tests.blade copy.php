...
<body class="bg-gradient-to-br from-[#f6f7ff] via-white to-[#f3f1ff] min-h-screen">

<!-- HEADER -->
<div class="p-4 bg-white/70 backdrop-blur-xl border-b flex justify-between items-center">
...
<div class="p-3 flex justify-center gap-2 bg-white/60 backdrop-blur-xl">
  ...
  <button onclick="setTab('scheduled',this)"
    class="tab px-4 py-2 rounded-full bg-scheduled/10 text-scheduled font-semibold">
    📅 مجدولة
  </button>
...
<!-- GLOBAL STATS -->
<div class="p-4">
  <div class="section bg-white/80 border">
    ...
    <div class="grid grid-cols-2 md:grid-cols-6 gap-3 text-sm">
      ...
      <div class="p-3 rounded-xl border bg-scheduled/5">
      ...
      <div class="p-3 rounded-xl border bg-indigo-50">
        <p class="text-gray-400">نسبة النجاح</p>
        <p id="successRate" class="font-bold text-dark">0%</p>
      </div>
    </div>
  </div>
</div>
...
<div class="p-4">
  ...
  <div class="section bg-white/80 border">
    ...
    <div class="flex flex-wrap gap-2">
      ...
      <input id="datePicker" type="date" class="border px-3 py-2 rounded-xl text-sm">

      <select id="driverFilter" class="border px-3 py-2 rounded-xl text-sm">
        <option value="">كل السائقين</option>
      </select>

      <select id="fromFilter" class="border px-3 py-2 rounded-xl text-sm">
        <option value="">من</option>
      </select>

      <select id="toFilter" class="border px-3 py-2 rounded-xl text-sm">
        <option value="">إلى</option>
      </select>

      <button onclick="resetFilters()" class="bg-dark text-white px-4 py-2 rounded-xl">
        إعادة ضبط
      </button>
...
const params = new URLSearchParams();
  ...
  if(datePicker.value)
    filters.push({key:'date', label:datePicker.value});
  ...
  activeFilters.innerHTML = filters.map(f => `
    <div class="chip bg-indigo-100 text-indigo-700 flex items-center gap-2">
      ${f.label}
...
function setTab(t,el){
  tab = t;
  document.querySelectorAll(".tab").forEach(b=>b.classList.remove("ring-2"));
...
function render(){
  ...
  container.innerHTML = Object.keys(grouped).map(sec=>`
    <div class="section bg-white/70 border">
