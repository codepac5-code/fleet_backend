...
function uploadImageAndGetUrl(UploadedFile $file, $modelType, $modelId, $collectionName = 'default')
{
...
function defaultSymbol(){
    return (new CurrencyChange())->defaultSymbol();
 }
...
function demoUserPermission(){
   return !(Auth::user()->hasAnyRole([Roles::Super_Admin->value]));
}


function dateTime_sitesetup($_datetime){
    $sitesetup = Setting::where('type','site-setup')->where('key', 'site-setup')->first();
    $datetime = json_decode($sitesetup->value);
    $date = date("$datetime->date_format / $datetime->time_format", strtotime($_datetime));
    return $date;
}
...
function getLoader(){
    return asset('images/loader.gif');
}
...
// function storeImage(Request $request, $serviceId){
...
function get_storage_image( $path){
...
function verify_office_document($provider_id){
...
function get_office_commission($bookings) {
...
function get_handyman_office_commission($handyman_id){
...
function get_user_active_plan($user_id){
    $get_office_plan  =  \App\Models\ProviderSubscription::where('user_id',$user_id)->where('status',config('constant.SUBSCRIPTION_STATUS.ACTIVE'))->first();
    $activeplan = null;
    if(!empty($get_office_plan)){
        $activeplan = new App\Http\Resources\API\ProviderSubscribeResource($get_office_plan);
    }
    return $activeplan;
}
...
function get_office_plan_limit($provider_id,$type){
    $limit_array = array();
...
function today_cash_total($user_id,$to='',$from='',$type = ''){
    ...
    if (auth()->user()->hasAnyRole(['provider'])) {
        $amount = \App\Models\PaymentHistory::where('receiver_id', $user_id)
        ->where('action', 'handyman_send_office')
        ->where(function ($query) use ($from, $to) {
            $query->where('status', 'pending_by_admin')
                ->orWhere('status', 'approved_by_office');
        })
        ->whereDate('datetime', '>=', $from)
        ->whereDate('datetime', '<=', $to)
        ->sum('total_amount');
...
function total_cash($user_id){
    ...
    if (auth()->user()->hasAnyRole(['provider'])) {
        ...
        $amount = \App\Models\PaymentHistory::where('receiver_id', $user_id)
        ->where(function ($query) {
            $query->where('action', 'handyman_send_office')
                ->where('status', 'approved_by_office')
